...
1. **Portée des Attributs de Classe (ou Variables d’Instance) :** Les attributs déclarés au niveau de la classe (en dehors des méthodes) peuvent avoir différents modificateurs d’accès :
...
1. **Portée des Attributs Statiques :** Les attributs déclarés avec le mot-clé static sont partagés entre toutes les instances de la classe.
...
5. **Utilisation Interdisciplinaire :** Bien qu’il soit principalement utilisé en ingénierie logicielle, UML peut également être appliqué dans d’autres domaines pour modéliser des systèmes complexes.
...
Pour en découvrir, je vous invite à visiter la page de wikipedia relative à l' [Unified Modeling Language (UML) : https://fr.wikipedia.org/wiki/UML_(informatique)](https://fr.wikipedia.org/wiki/UML_(informatique)).

## Conclusion

Nous avons fait un tour, certes rapidement, de ce que propose le langage Java et la modélisation UML.
Nous utiliserons donc le langage Java, et l’UML pour expliquer un peu les approches de conception du code proposé.
