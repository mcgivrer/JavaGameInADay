...
. Nous instancions l’implémentation désirée de l’interface `GameLoop`, ici, la class `StandardGameLoop2` dont nous verrons le code juste après,
...
. Initialisation de l’implémentation de la boucle de jeu (`StandardGameLoop2`)en lui passant l’instance parente du jeu (`Game`),
