...
2. ***viewport*** : le cadre de a caméra,
...
![Illustration de la caméra fixant une cible ayant un cadre et un facteur de suivi](illustrations/game101_camera.png)
...
![capture-camera-1](illustrations/capture-camera-1.png)
...
![Affichage d'informartion visuel de debuggage sur l'objet Camera](illustrations/capture-camera-2.png)
...
2. La gravité (`gravity`), nouvelle venue dans notre moteur de jeux, servira à appliquer une force définie à l’ensemble des entités contenues dans l’espace de jeu correspondante à la gravité de notre environnement.
Par défaut à zéro, elle pourra être définie à __9,81m/s^2^__, soit l’accélération liée à [la gravité terrestre,role=external,window=_blank,title=Voir la page sur la pesanteur sur wikipedia](https://fr.wikipedia.org/wiki/Pesanteur).
...
**💡 TIP**\
TODO Ajouter un schema montrant quelques entités évoluant dans une zone de jeux et en subissant les contraintes
...
Ici, nous définissons un monde délimitant une zone de jeu de ***480*** points en largeur et ***320*** points en hauteur, ainsi qu’une gravité définie via une instance de [`Point2D`](https://docs.oracle.com/en/java/javase/23/docs/api/java.desktop/java/awt/geom/Point2D.html) de __(0.0,-0.981)__, soit la gravité terrestre de ***9,81m/s^2^***.

**📌 NOTE**\
Nous utilisons ici un object de type Point2D, fourni par le JDK afin de définir un pseudo vecteur.
...
1. Dans la méthode `render`, nous passons maintenant l’objet World issue de la scene :  `currentScene.getWorld()`
