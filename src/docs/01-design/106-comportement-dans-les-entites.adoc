...
NOTE: Je vous propose de créer un petit script _bash_ `run.sh` pour exécuter n'importe quel exemple:

.Script 'run.sh' d'exécution de la compilation et d'exécution d'un exemple
[source,bash]
include::../../../run.sh[Script 'run.sh' d'exécution de la compilation et d'exécution d'un exemple]

Ce qui sera exécuté par :

[source,console]
----
./run.sh MonProgrammeBehavior1
----
