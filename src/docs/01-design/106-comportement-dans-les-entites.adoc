...
[source,java]
----
public class examples.MonProgrammeBehavior1 extends JPanel implements KeyListener,Game {
    //...
...
.la ligne de commande ultime qui permet de compiler et d’exécuter l’un de nos exemples, ici 'examples.MonProgrammeBehavior1'

.Exécution des codes d'exemples dans le répertoire _src/test_
[source,bash]
----
javac -d target/test-classes $(find src/test -name "*.java") && find src/test -name "*.properties" -exec cp --parents {} target/test-classes \; && java -cp target/test-classes examples.MonProgrammeBehavior1
----
...
[source,console]
----
./runex.sh examples.MonProgrammeBehavior1
----
