...
![L'écran cathodique](illustrations/ecran-cathodique.png)
...
![Eclaté technique de l'écran cathodique](illustrations/ecran-cathodique-eclate.png)
...
3. `update` lance la mise-à-jour de la scene active,
4. `render` procède au rendu de la scene active,
5. `setExit` permet de définir une demande de sortie de l’exécution de la boucle,
6. `setPause` demandera la mise en pause de l’exécution de la methode `update`.
...
1. Nous instancions l’implémentation désirée de l’interface `GameLoop`, ici, la class `StandardGameLoop2` dont nous verrons le code juste après,
...
![capture-gameloop-delegation-1](illustrations/capture-gameloop-delegation-1.png)
...
![La boucle de jeu standard (image inspirée du livre "Game Programming Patterns" de Robert Nystrom.)](illustrations/game-loop-standard.jpg)
...
1. Initialisation de l’implémentation de la boucle de jeu (`StandardGameLoop2`)en lui passant l’instance parente du jeu (`Game`),
