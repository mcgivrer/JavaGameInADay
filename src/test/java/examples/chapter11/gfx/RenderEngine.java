...
/**
 * The {@link RenderEngine} introduce all the drawing process for every {@link Entity2} on a {@link Scene}.
 *
...
public class RenderEngine {
    ...
    public void render(Scene currentScene) {
        ...
        Optional<Entity2> cam = currentScene.getEntities().stream().filter(e -> e instanceof Camera).findFirst();
        ...
        currentScene.getEntities().stream()
                ...
                .forEach(e -> {
                    ...
                    e.getBehaviors().forEach(b -> ((Behavior2<Entity2>) b).draw(g, e));
                });
        g.dispose();
    ...
    private void drawEntity(Entity2 e, World world, Graphics2D g) {
        ...
        g.drawLine((int) (e.getShape().getBounds().width * 0.5), (int) (e.getShape().getBounds().height * 0.5),
                (int) (e.getShape().getBounds().width * 0.5 + e.getVelocity().x * 4), (int) (+e.getShape().getBounds().height * 0.5 + e.getVelocity().y * 4));
        drawDebugEntity(g, world, e);
        g.translate((int) -e.getX(), (int) -e.getY());
    }

    private void drawDebugEntity(Graphics2D g, World world, Entity2 e) {
        if (isDebugGreaterThan(1)) {
            ...
            if (isDebugGreaterThan(2)) {
                ...
                if (isDebugGreaterThan(3)) {
                    ...
                    g.drawString("g:%3.2f,%3.2f".formatted(world.getGravity().x, world.getGravity().y), (int) e.getWidth(), 40);
                }
            }
        }
    }
