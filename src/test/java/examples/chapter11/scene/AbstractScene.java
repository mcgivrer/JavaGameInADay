...
public abstract class AbstractScene extends Node<Scene> implements Scene {
     ...
     */
    protected World world = new World(new Point2D.Double(0, -0.981), new Double(0, 0, 640, 200));
     ...
     */
    // <2>
    public Collection<Entity2<?>> getEntities() {
        return getChildren().stream()
                .filter(Entity2.class::isInstance) // Filtrer les objets de type Entity
                .map(Entity2.class::cast)         // Les convertir en Entity
     ...
     */
    // <3>
    public Entity2<?> getEntity(String name) {
        return (Entity2<?>) getChildren().stream().filter(c -> c.getName().equals(name)).findFirst().get();
    }
